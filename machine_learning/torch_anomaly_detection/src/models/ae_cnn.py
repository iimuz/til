...
class CR2d(nn.Module):
    def __init__(
        ...
        if use_activation:
            self.conv.add_module("activation", nn.LeakyReLU(inplace=True))

    def forward(self, x):
        return self.conv(x)


class SimpleCBR(nn.Module):
    """SimpleCR に Batch Normalization を追加."""

    def __init__(self, in_channels: int, out_channels: int,) -> None:
        super(SimpleCBR, self).__init__()

        self.channels = np.array([32, 64, 128]) * 4
...
class SimpleCR(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,) -> None:
        super(SimpleCR, self).__init__()

        self.channels = np.array([32, 64, 128, 64, 32])

        self.encoder = nn.Sequential(
            CR2d(in_channels, self.channels[0], (3, 3), padding=1),
            nn.MaxPool2d((2, 2), stride=2),
            CR2d(self.channels[0], self.channels[0], (3, 3), padding=1),
            nn.MaxPool2d((2, 2), stride=2),
            CR2d(self.channels[0], self.channels[0], (3, 3), padding=1),
            CR2d(self.channels[0], self.channels[1], (3, 3), padding=1),
            nn.MaxPool2d((2, 2), stride=2),
            CR2d(self.channels[1], self.channels[1], (3, 3), padding=1),
            CR2d(self.channels[1], self.channels[2], (3, 3), padding=1),
            nn.MaxPool2d((2, 2), stride=2),
            CR2d(
                self.channels[2],
                self.channels[2],
            ...
            CR2d(self.channels[2], self.channels[3], (3, 3), padding=1),
            CR2d(self.channels[3], self.channels[4], (3, 3), padding=1),
            CR2d(
                self.channels[4], out_channels, (3, 3), padding=1, use_activation=False
            ),
        )
        self.decoder = nn.Sequential(
            CR2d(out_channels, self.channels[4], (3, 3), padding=1),
